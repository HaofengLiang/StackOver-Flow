...
mongoose.connect('mongodb://64.52.86.95:27018/personal', { useNewUrlParser: true, useCreateIndex: true });
...
app.post('/adduser', async function(req, res){
   logger.info("adding one user by req.body.username" + req.body.username);
   try{
      ...
      logger.info('create a new disabled uesr successfully! with Name:'+ saveUser.username + ', Email:'+ saveUser.email);
      mailOptions = {
...
app.post('/login', async function(req, res){
    logger.info("Get session in login"+ JSON.stringify(req.signedCookies));
    try{
        let logged_user = await User.findOne({username: req.body.username, password: req.body.password, verify:true});
        if(logged_user == null){
...
app.post('/logout', async function(req, res){
  logger.info('Session info in logout: '+ JSON.stringify(req.signedCookies));
    try{
...
app.get('/user/:username', async function (req, res) {
  try{
      var getUser = memcached.get(req.params.username);
      if(getUser){
        logger.info("Get user info from memcaced: "+ req.params.username);
        ...
        if(getUser == null){
          ...
          await memcached.set(req.params.username, getUser, 300) // keep 5 minutes
          logger.info("Get User' s information: "+ req.params.username);
          res.json({ status: 'OK', user: { email: getUser.email, reputation: getUser.reputation } })
        }
      }
    }catch(err) {
...
app.get('/user/:username/questions', async function (req, res) {
  try{
      var getQuestions = memcached.get(req.params.username + "getQ");
      if(getQuestions){
           res.json({ status: 'OK', questions: getQuestions });
      }else{
         var getUser = await User.findOne({username: req.params.username});
         if(getUser == null){
            logger.error("You did not find any valid user to get question: "+ req.params.username);
           ...
           await memcached.set(req.params.username + "getQ", getQuestions, 300) // keep 5 minutes
           res.json({ status: 'OK', questions: getQuestions });
         }
      }
    }catch(err){
...
app.get('/user/:username/answers', async function (req, res) {
  try{
    var getAnswers = memcached.get(req.params.username + "getA");
    if(getAnswers){
       ...
       if(getUser == null){
         ...
         await memcached.set(req.params.username + "getA", getAnswers, 300) // keep 5 minutes
         res.json({ status: 'OK', answers: getAnswers });
       }
    }
  }catch(err){
